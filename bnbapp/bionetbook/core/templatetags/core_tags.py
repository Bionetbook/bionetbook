...
def object_data_table(model_instance):
    ...
    for field_name in model_instance._meta.get_all_field_names():
        if field_name not in ['data', 'actions', 'components', 'thermocycle', 'machine', 'actions', 'slug', 'name', 'component - list', 'objectid', 'protocols']:  # USED TO NOT DISPLAY RAW DOCUMENT DATA
            try:
