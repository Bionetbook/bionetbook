...
urlpatterns = patterns("",
    ...
    url(regex=r'^(?P<protocol_a_slug>[-\w]+)/layer/(?P<layers>[-\w]+)/$', view=views.SingleLayersView.as_view(), name='add_single_layers'),
    url(regex=r'^(?P<protocol_a_slug>[-\w]+)/layer/(?P<layers>[-\w]+).(?P<format>[-\w]+)$', view=views.SingleLayersGraphicView.as_view(), name='add_single_layers_graphic')
