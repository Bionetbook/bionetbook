...
/*
 * Search for "Custom Edit Area" for modified areas of code
 */
...
function AgendaWeekView(element, calendar) {
	...
	function render(date, delta) {
		...
		//////////////////////////////////
		//  Custom Edit Area  			//
		//  Add data to the Week View   //
		//////////////////////////////////
		var weekDays = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"];

		// Add date reference to first column(sunday)
		$(".fc-col0").attr("data-date", start)

		// Add day elements to the empty row time slots in week view
		for(var i = 0; i < 48; i++){

			// Clear all previous data
			$("tr.fc-slot" + i + " .fc-widget-content > div:first-child").html("");

			// Add an element for each day to the 30 min slots
			for(var j = 0; j < 7; j++){
				var dayNode = document.createElement("div"),
					dateData = new Date(new Date(start).setDate(start.getDate() + j));

				dateData.setHours(Math.floor(i/2));
				dayNode.className = "week-day";
				dayNode.setAttribute("data-day", weekDays[j]);
				dayNode.setAttribute("data-date", dateData)

				$("tr.fc-slot" + i + " .fc-widget-content > div:first-child").append(dayNode);
			}
		}
	}
	...
	function draggableSlotEvent(event, eventElement, timeElement) {
		...
		eventElement.draggable({
			...
			stop: function(ev, ui) {
				//////////////////////////////////////////////////
				//  Custom Edit Area 	 						//
				//  Weekly view stopDrag event listener 		//
				//////////////////////////////////////////////////
				...
				if(!stepsAreInOrder && !isLocked) BNB.calendar.displayTopError("Oops, you can't drag your steps out of order!");

				var cell = hoverListener.stop();
				clearOverlays();
				trigger('eventDragStop', eventElement, event, ev, ui);

				if ((stepsAreInOrder || isLocked) && cell && (dayDelta || minuteDelta || allDay)) {
					// changed!
